...
- Permission overwrites, role permission changes, guild-level security settings, and blueprints containing those fields are privileged writes. They require full mode and a one-time, expiring, payload-bound confirmation returned by the matching dry-run without enabling destructive actions.
...
- Guild prune execution and bulk bans require full mode, destructive opt-in, bulk limits, and a one-time operation-specific confirmation.
